---
name: zanmai:voice
description: Turn spoken notes in `inbox/` into what they were meant to be. Triggers on `/zanmai-voice`, on the hook reporting recordings, or on "I recorded something".
---

# voice

Speaking is the cheapest way to get something into a space, and the transcript is the expensive part. The words that carry the meaning are the names, and names are what speech recognition mangles. Putting them right is reading, not machinery: a garbled word resolves from the sense of the sentence the way a typo does, and most of the world's names are already known to whoever is reading. That is the default path, and for most notes it is the only one.

Two things are worth knowing about the rest.

**A word only the space could know needs the space.** A colleague's surname, a company spelled a particular way, a term nobody outside this space uses. `zanmai.py voice lexicon` collects those and hands them to the recogniser as its initial prompt, which is the one thing that can recover a misheard name, because it is the only step that hears the audio again. It is reached for when reading does not get there, not run as a matter of course.
...
**Then fix what you can see is wrong**, and settle how each name is written. For a name the space holds, the file that owns it is the authority, in full: case, hyphens, spacing. A recogniser applies its own capitalisation and cannot be talked out of it, so this part is never the recogniser's to decide.

**Where you are stuck on a name, then reach for the space's list and run it again.** Not before: most notes never need it.
...
Read all of them first, then decide, then file. What a note is comes from what it says and from the space it landed in, not from a list of types: how the day went belongs in the periodic note as spoken, an instruction is carried out as if it had been typed, four sets of squats belong wherever this person's training already lives, an errand belongs where their errands are, and an idea belongs to the bundle it is about. None of that is recognisable from the wording, all of it from the context, which is why there is no table here.
...
**When nobody is at the keyboard.** The run can be started on a schedule, and then the invocation says so. Nothing about the reading changes; three things about the ending do. Open points go to `zanmai.py work ask` instead of a question, because a question asked into an empty room stops the run for a day. The run does not park (operating-principles principle:parking): nobody is there to wake it. And it closes itself through the `close-session` skill, which writes the log with `session_type: unattended`, so the next real session is told in one line what happened and what is waiting. Decisions are taken rather than deferred, in the knowledge that the report makes them easy to undo.

## Sounds sensible, is wrong

| Sounds reasonable | Why it is wrong |
|---|---|
| "The space does not have it, so it cannot be resolved." | Most of what a note names is not space material at all. You know most of it, and the web has the rest. The space is for what only this person could have. |
...
| "While I am here I will write up what I found." | One line in the right bundle, so the next note is free. A write-up is a different job and nobody asked for it. |
...
| "No model, so I will use an online service." | A voice note is private material. Missing tools are a stop, not a detour (operating-principles principle:tool-presence). |

## Stop and look again

- About to file a note under a name that is not in the space and was not looked up.
