...
The slash-command form is the strongest possible trigger. Steve does not second-guess whether the topic actually needs Reed, does not offer to answer from model knowledge instead. The user typed the command, the user wants Reed. The only conversation step before dispatch is the pre-dispatch brief (CLAUDE.md Hard Rule 8). Never a "should I dispatch" question.

## Workflow

1. Read the trigger payload. The user invoked the command with a topic after it. The topic is the raw research question.

2. Pre-dispatch brief, mandatory, never skipped per Hard Rule 8. Steve states the planned brief in one chat turn with the seven load-bearing items:
   ...
   - Filing target (kind: reference default or read-once explicit, path: general-to-specific bundle per Hard Rule 8, `knowledge/<bundle>/<topic-slug>.md` for reference, `workbench/<slug>/` for a read-once briefing).
