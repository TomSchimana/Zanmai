...
- **Then the run reads the survey, not the files.** For the decision at this point, which is what
  each thing is and where it belongs, the survey is usually enough: a paper that carries a date, an
  amount and a company name is that company's invoice from that date. **Open a file only where the
  survey leaves the question open**, and name in the return which ones those were and why. A file
  the machine could not read at all is always one of them.
- **Guessing from the filename is not reading either.** A screenshot can be anything; what it is
  stands in the picture, not in its name.
- **It checks a tool before it needs it**, with `zanmai.py tools check <id>`, and returns the gap
  rather than fetching. A fetch is the user's call and it has a size.
- **What it returns, per file**: what the thing is, in one line, what it appears to be for, and
  where it would go. Plus, once: what is still open.
- **It reads the routing table first**, `zanmai.py import scan` prints the rule per file. A rule
  keys on what something is, not on the file it arrived as: a file type says how to read something
  and nothing about what it is for. Where the user has already said where this sort of thing
  belongs, that is the answer and asking again asks them to repeat themselves. Where a file matches
...
Then Steve reports that and asks. The filing run below happens after the user has answered, and it
is the run all the directives above are written for.
...
- A single-line capture into a journal entry (the `journal` skill handles that).
...
Run this once per bundle after `asset add` is done for every attachment of that bundle. Skipping this step is what makes an embed show up as literal link text: the editor cannot resolve the file and falls back to the embed source string.
...
1. Tooling-gap note. If Hank had to work around a missing subcommand, a hook edge case or unexpected behaviour during the run, mention it in one final user-facing line as a statement (in the user's writing language, naming what was missing and how it was worked around, plus that it has been logged). Append a detailed entry to `zanmai/logs/<YYYY>/<MM>/builder-gaps.md` in the same turn. Never make this a user question.
...
- About to call `Write` or `Edit` on a file inside a bundle. Use `zanmai.py` instead.
