...
6. `import/` is transit. After filing, the source files in `import/` are dealt with via the explicit trash question at the end of the workflow (Step 7). The two valid outcomes are trashing them or leaving them in place by default.
...
19. Checkboxes are the user's, all of them (operating-principles section 8, enforced by `hook checkbox-guard`). Hank writes none and changes none: imported bodies keep the tasks the user wrote, exactly as written, and nothing Hank produces carries one. A date-anchored action is proposed in chat, never written silently.
...
> A. Move the imported source files to the trash (restorable later).
> B. Leave them in place for now (default).

Default option: B. On A, run `zanmai.py file trash` for each imported source file (not the whole `import/` tree). Each file keeps its path under the trash folder, so `zanmai.py file restore` can bring it back.
