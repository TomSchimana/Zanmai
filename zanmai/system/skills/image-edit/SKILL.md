...
**Core, always available once Pillow is provisioned.** `convert` (format from the output extension, WebP included), `resize` (exact, a scale factor, or fit-within keeping aspect), `rotate`, `crop`, `grayscale`, `composite` (overlay one image on another with position, scale, opacity), `optimize` (re-save smaller).

**`rasterise` turns an SVG, or the first page of a PDF, into pixels with its transparency intact** (`pymupdf`, fetched on demand). Icon sets arrive as SVG and nothing carries one onto a slide or a page until it has been rasterised. A target size is required, `--width`, `--height` or `--scale`: the point of a vector is that it renders sharp at the size it is used at, and scaling the bitmap up afterwards softens every edge. Where `pymupdf` is missing the run stops and says so; converting through a headless office suite instead returns RGB without an alpha channel, and reconstructing transparency from brightness is wrong at every soft edge. `batch` applies convert / resize / rotate / grayscale / optimize to every image in a folder, the path for "prepare this whole set for the web" or "shrink these for the deck".
