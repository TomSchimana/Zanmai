...
Variants render to your work folder; the chosen asset goes to `doing/<slug>/`, or into a design piece, where it sits in that piece's own bundle. A provenance sidecar, prompt, references, model, parameters, travels with it so the result is reproducible. Show, then deliver on a yes; never write straight to the final place.
