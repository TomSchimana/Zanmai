...
- `write`: slash command `/zanmai-write`, and pulled by every expert who produces text, Hank, Reed, Carol and Steve. Anything longer than a line that gets written for the user runs through it. It sets four things before the first sentence, the valid source, the purpose and its readers, the frame and its bans, the format, and it looks in the vault for a comparable document to take structure and voice from rather than inventing a style. Whatever the brief left open is not asked as a question: it is proposed in one line the user can veto, because a proposal is worth more than a form. It is registered with the host rather than only read as a file, which is what makes its `model:` take effect, so a session running light still writes properly. It holds the rules for a document in which other people appear by name, which is where an unbriefed summary does its damage.
