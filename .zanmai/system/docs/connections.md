...
Read-only is the spirit: a connection reads the source and answers in chat, nothing is written to the source, nothing copied into your files by Wong. A read that should become a note goes to Hank. Sync and write-back are out of scope; a real need would be a separate opt-in extension, not the core.
