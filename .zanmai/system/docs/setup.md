...
4. `zn` CLI. Only checked if Case A or Case-B-yes (ZenNotes is intended for this vault). Optional. The flag goes to `true` only when the binary is installed *and* `.zennotes/vault.json` exists, meaning ZenNotes has opened this vault and the CLI acts on it rather than on its own default vault. Without a usable CLI, `zanmai.py` falls back to Unix `mv` for trash and archive (loses the ZenNotes-restore-path semantics).
