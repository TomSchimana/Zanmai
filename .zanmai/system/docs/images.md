...
Variants are rendered into a scratch area and shown to you first. The one you pick moves to `_export`, or into a design piece through the shared `assets` folder. The prompt, the references, the model and the parameters travel with the file, so a later variant inherits the right anchors instead of losing them.
