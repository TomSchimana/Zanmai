...
Record a voice note, drop it in `_import/recordings/`, and it gets read. A thought on the way to the car, a journal entry at the end of the day, an instruction you would rather say than type. The folder is created for you the first time.
...
**The recording itself is kept**, in `assets/recordings/`, by year and month. It moves out of the drop folder so it cannot be read twice, and it is not deleted: it is yours, and a transcript is a reading of it.
