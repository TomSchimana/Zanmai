...
- **`snapshots`**. Each one is a full copy of the vault. Syncing them puts a backup inside your backup, and it is by far the largest thing in the folder.
...
That is what snapshots are for, and it is why they are worth keeping out of the sync: a timestamped copy taken before anything risky, sitting on the machine, restorable in one step. [Snapshots and going back](snapshots.md) covers them.
