...
Zanmai takes what is on your mind off it, and does more than remember it. It orders, thinks, creates and gets things done. It holds what does not have to stay in your head:
...
2. Four slash commands cover the most common operations:
...
When the user picks a number or asks about a topic's state, read the bundle truth file (`inbox/<kind>/<bundle-slug>/<bundle-slug>.md`) before claiming an item is open. The truth file outranks the Daily-Note checkbox; if the bundle says done, name that and offer to tick the todo.
