...
2. Pre-dispatch brief, mandatory, never skipped per Hard Rule 9. Steve states the planned brief in one chat turn with the seven load-bearing items:
   ...
   - Filing target (kind: reference default or read-once explicit, path: general-to-specific theme-bundle per Hard Rule 9, `inbox/knowledge/<theme>/<topic-slug>.md` for reference, `inbox/review/<slug>.md` for a read-once briefing).
...
7. On the user's yes to the open offer, the file opens via `zn open <path>` when `zen_cli_installed: true`, otherwise via the platform fallbacks listed in Steve's contract.
...
The skill is registered as two slash commands.

- `/zanmai:research`: canonical English.
- `/zanmai:recherche`: alias, same workflow, for users writing in a German setting.
