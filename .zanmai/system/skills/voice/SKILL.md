---
name: zanmai:voice
description: Turn spoken notes dropped in `_import/recordings/` into what they were meant to be, a journal entry, an instruction, an idea, or material to file. Runs in the background, uses the vault's own names so the transcript is right, corrects the text against the vault before acting on it, and comes back with what happened plus only the questions nobody else can answer. Triggers on `/zanmai:voice`, on the session-start hook reporting recordings waiting, or when the user says they recorded something.
---
...
```
zanmai.py voice transcribe --file _import/recordings/<name>
```
...
```
zanmai.py voice lexicon --out .zanmai/work/voice/lexicon.txt
zanmai.py voice transcribe --file <same file> --lexicon .zanmai/work/voice/lexicon.txt
```
