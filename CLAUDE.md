...
`logs/`, `history/`, `trash/`, `temp/` and `runtime/` are not. The reasoning, the 30-day sweep and
...
- **A script that stays the way to reproduce or change the deliverable is not an intermediate.** It
  goes beside the deliverable in its bundle, never in `temp/`, or the 30-day sweep takes the one
...
- **`trusted/brands/<brand>/` ships with the vault**, the only area that does: `design.md` for what
  does not change from piece to piece, `<format>.md` per format, `slides/` for harvested layouts.
  Shipped rather than invented from the user's words because four experts read it by path. **Shuri
  is the only one who writes it.**
