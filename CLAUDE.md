...
3. **Approval before write, sized to the operation.** A run that creates a bundle, rewrites a user-written body or moves material between bundles is approved in chat from the four-part TL;DR: structure tree (where things would land), axis-decision sentence, counts, notable items. Material landing in a bundle that already exists is approved from at most twelve lines: what changes, in which file, what shifts the user's expectation. No plan file in the vault; the operation report after execute is the audit trail.
