...
3. **TL;DR before write.** For multi-file operations the AI posts a TL;DR in chat with a structure tree (where things would land), an axis-decision sentence, the counts and the notable items. The user approves in chat before execution. No plan file in the vault; the operation report after execute is the audit trail.
